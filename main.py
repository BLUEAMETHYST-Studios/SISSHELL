...
os.system("cls")
os.system("title SISSHELL Beta 1.2 (Loading ...)")
...
def checkAdmin():
    if not os.path.exists("C:/Users/" + USERNAME + "/AppData/Local/SISSHELL/master-data.txt"):
        ...
        os.system("title SISSHELL » Login")
...
print(Fore.LIGHTGREEN_EX + "SISSHELL " + Fore.LIGHTBLUE_EX + "Beta v.1.2")
print(Fore.LIGHTGREEN_EX + 'Type "help" for assistance!')
print("\n")


while True:
    os.system("title SISSHELL Beta 1.2 » " + os.getcwd())
    ...
    if SISSHELL == "help":
        ...
        print(Fore.LIGHTBLUE_EX + "  / ____|_   _|/ ____/ ____| |  | |  ____| |    | |     " + Fore.LIGHTGREEN_EX + "  Version: Beta 1.2") 
    ...
    if SISSHELL == "title-reset":
        ...
        print(Fore.CYAN + "Network Adapters: " + Fore.LIGHTGREEN_EX + "\n==========================================================\n\n" + Fore.LIGHTBLUE_EX + NICSTR + Fore.LIGHTGREEN_EX + "\n==========================================================\n")
    try:
    ...
    if SISSHELL.startswith("webrq "):
        WEBRQ = SISSHELL[6:]
        if SISSHELL == "webrq -help" or SISSHELL == "webrq -?":
            print(Fore.LIGHTBLUE_EX + """
Usage:
...
-allowrd | Allows Redirects by the HTTP Server      
""")
        else:
            try:
                subprocess.run(["curl", WEBRQ, "-s"])
            except WEBRQ.endswith("-allowrd"):
                WEBRQ = WEBRQ.strip("-allowrd")
                subprocess.run(["curl", WEBRQ, "-L", "-s"])
            except Exception:
                print(Fore.RED + "Something went wrong during the execution of this command!")
            else:
                print(Fore.LIGHTBLUE_EX + "Web Request sended!")
                
    elif SISSHELL.startswith("timeaction "):
        TIMEACTION = SISSHELL[11:]
        if SISSHELL == "timeaction -help" or SISSHELL == "timeaction -?":
            print(Fore.LIGHTBLUE_EX + """
Usage:

timeaction ({calculation}) {amount}

Example:

timeaction (10 * 10) 1000      
""")
        else:
            try:
                calculation, amount = TIMEACTION.split(")")
                ...
                print(Fore.LIGHTBLUE_EX + f"It took {total_time} to complete the action!")
            except ZeroDivisionError:
                print(Fore.RED + "Division by Zero isn't possible!")
            except ValueError:
                print(Fore.RED + "At least one value was invalid!")
            except OverflowError:
                print(Fore.RED + "Due to limitations it is not possible to calculate this!")
            except Exception:
                print(Fore.RED + "Something went wrong during the execution of this command!")
